...
function wpape_gallery_manage_field( $field, $meta ) {
	...
	echo '<div class="wpape-gallery-manage apeGalleryDiv " '.($autoOpen?' data-open="1"':'').' style="text-align: center;">';
	echo '	<input type="hidden" class="wpape-gallery-manage-value" id="' . $field->args( 'id' ) . '" name="' . $field->args( 'id' ) . '" value="' . $meta . '" />';
	echo '	<button class="btn btn-success btn-lg "><span class="glyphicon glyphicon glyphicon-th" aria-hidden="true"></span> ' . __( 'Upload / Edit Images' ,'gallery-images-ape' ) . ' </button>';
	echo '</div>';
	echo '<p class="cmb2-metabox-description">'.__('Images settings: <b>Links, Video Links</b> and etc.','gallery-images-ape') . '</p>';
}
